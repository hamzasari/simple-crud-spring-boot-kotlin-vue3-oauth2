...
export default {
  ...
  methods: {
    ...
    handleDoneEditing() {
      this.editing = false
      this.$api.updateForId(this.item._links.todo.href.split('/').pop(), this.editingTitle, this.item.completed).then((response) => {
        this.setTitle(this.item._links.todo.href, this.editingTitle)
    ...
    handleClickSetCompleted(value) {
      this.$api.updateForId(this.item._links.todo.href.split('/').pop(), this.item.title, value).then((response) => {
        this.setCompleted(this.item._links.todo.href, value)
        this.$log.info('Item updated:', response.data);
      }).catch((error) => {
    ...
    handleClickDelete() {
      this.deleteMe(this.item._links.todo.href.split('/').pop())
    }
  }
}
</script>
